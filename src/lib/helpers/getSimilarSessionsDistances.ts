...
export function getSimilarSessionsDistances(
  meal: Meal,
  sessions: Session[],
): SessionAndScore[] {
  ...
  // Compute scale per food, INCLUDING targetMap in the dataset
  const allMaps = [targetMap, ...sessionMaps];
  const foodScales = new Map<string, number>();

  for (const name of allFoodNames) {
    const amounts = allMaps.map((map) => map.get(name) ?? 0);
    const mad = MathUtil.meanAbsoluteDeviation(amounts);

    // Default fallback minimum scale (e.g., 1.0 or average portion size floor)
    foodScales.set(name, Math.max(mad, 1.0));
  }
  ...
  return sessionsDistances.sort((a, b) => a[1] - b[1]);
}
